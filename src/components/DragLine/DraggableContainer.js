...
/**
 * 问题：拖拽的元素 z-index需要高一点，拖拽结束怎么办，怎么实现新拖拽的元素的优先级悬浮在后拖拽的上面
 */
      ...
      <Container>
        {vLines.map(({ length, value, origin }, i) => (
          <div
            className="v-line"
            key={`v-${i}`}
            style={{ left: value, top: origin, height: length, width: 0, ...commonStyle }}
          />
        ))}
        {hLines.map(({ length, value, origin }, i) => (
          <div
            className="h-line"
            key={`h-${i}`}
            style={{ top: value, left: origin, width: length, height: 0, ...commonStyle }}
          />
        ))}
      </Container>
    )
